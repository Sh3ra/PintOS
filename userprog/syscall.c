...
static uint32_t write(int fd, void *buffer, unsigned int size)
{
    ...
    else if (fd == NULL || fd == 0 || list_empty(&thread_current()->my_opened_files_list))
        return 0;
    else
    {
...
static void seek(int fd, unsigned position)
{
    if (fd == NULL || fd < 2 || list_empty(&thread_current()->my_opened_files_list))
        return;
    struct list_elem *file_elem = list_front(&thread_current()->my_opened_files_list);
    file_elem = move_elem_ptr(file_elem, fd);
    if (file_elem != list_tail(&thread_current()->my_opened_files_list))
...
static void close_file(int fd)
{
    if (fd < 2 || list_empty(&thread_current()->my_opened_files_list))
        return;
    struct list_elem *file_elem = list_front(&thread_current()->my_opened_files_list);
    file_elem = move_elem_ptr(file_elem, fd);
    if (file_elem != list_tail(&thread_current()->my_opened_files_list))
